...
/**
  ...
  * dtmi:appconfig:steval_mkboxpro:fp_atr_ble1:sensors:lps22df_press;1
  *
...
uint8_t Lps22df_Press_PnPL_vtblGetStatus(IPnPLComponent_t *_this, char **serializedJSON, uint32_t *size, uint8_t pretty)
{
  ...
  float temp_f = 0;
  lps22df_press_get_thresholds_usage(&temp_f);
  uint8_t enum_id = 0;
  ...
  json_object_dotset_number(JSON_Status, "lps22df_press.thresholds_usage", enum_id);
  lps22df_press_get_threshold_low(&temp_f);
  json_object_dotset_number(JSON_Status, "lps22df_press.threshold_low", temp_f);
  lps22df_press_get_threshold_high(&temp_f);
  json_object_dotset_number(JSON_Status, "lps22df_press.threshold_high", temp_f);
...
uint8_t Lps22df_Press_PnPL_vtblSetProperty(IPnPLComponent_t *_this, char *serializedJSON)
{
  ...
  if (json_object_dothas_value(tempJSONObject, "lps22df_press.thresholds_usage"))
  {
    int thresholds_usage = (int)json_object_dotget_number(tempJSONObject, "lps22df_press.thresholds_usage");
    switch (thresholds_usage)
    {
      case 0:
        lps22df_press_set_thresholds_usage(external_range);
        break;
      case 1:
        lps22df_press_set_thresholds_usage(internal_range);
        break;
      case 2:
        lps22df_press_set_thresholds_usage(less_than);
        break;
      case 3:
        lps22df_press_set_thresholds_usage(bigger_than);
        break;
    }
  }
  if (json_object_dothas_value(tempJSONObject, "lps22df_press.threshold_low"))
  {
    float threshold_low = (float) json_object_dotget_number(tempJSONObject, "lps22df_press.threshold_low");
    lps22df_press_set_threshold_low(threshold_low);
  }
  if (json_object_dothas_value(tempJSONObject, "lps22df_press.threshold_high"))
  {
    float threshold_high = (float) json_object_dotget_number(tempJSONObject, "lps22df_press.threshold_high");
    lps22df_press_set_threshold_high(threshold_high);
  }
  json_value_free(tempJSON);
  return ret;
}
