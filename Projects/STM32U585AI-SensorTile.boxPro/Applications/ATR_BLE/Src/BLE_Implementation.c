...
void InitPnPLComponent(void)
{
  ...
  pStts22hVsPnPLObj = Stts22h_Temp_PnPLAlloc();
  pControlPnPLObj = Control_PnPLAlloc();
  pLsm6dsv16xMCLVsPnPLObj = Lsm6dsv16x_Mlc_PnPLAlloc();
  pLsm6dsv16x6DVsPnPLObj = Lsm6dsv16x6d_Acc_PnPLAlloc();
  pLsm6dsv16xWUVsPnPLObj = Lsm6dsv16xwu_Acc_PnPLAlloc();
  pLps22dfVsPnPLObj = Lps22df_Press_PnPLAlloc();
  pDeviceInformationPnPLObj = Deviceinformation_PnPLAlloc();

  /* Init&Add PnP-L Components */
  Stts22h_Temp_PnPLInit(pStts22hVsPnPLObj);
  Control_PnPLInit(pControlPnPLObj, &iControl);
  Lsm6dsv16x_Mlc_PnPLInit(pLsm6dsv16xMCLVsPnPLObj);
  Lsm6dsv16x6d_Acc_PnPLInit(pLsm6dsv16x6DVsPnPLObj);
  Lsm6dsv16xwu_Acc_PnPLInit(pLsm6dsv16xWUVsPnPLObj);
  Lps22df_Press_PnPLInit(pLps22dfVsPnPLObj);
...
void BLE_SetCustomAdvertiseData(uint8_t *manuf_data)
{
  if (FinishGood == FINISHA)
  {
    manuf_data[BLE_MANAGER_CUSTOM_FIELD1] = ATRBLE1A_BLUEST_SDK_FW_ID;
  }
  else
  {
    manuf_data[BLE_MANAGER_CUSTOM_FIELD1] = ATRBLE1B_BLUEST_SDK_FW_ID;
  }
...
/**
  ...
  * @param  uint32_t length lengt of string to write
...
static uint32_t DebugConsoleCommandParsing(uint8_t *att_data, uint8_t data_length)
{
  ...
  else if (!strncmp("info", (char *)(att_data), 4))
  {
    ...
    BytesToWrite = sprintf((char *)BufferToWrite, "\r\nSTMicroelectronics %s:\n"
                           "\tVersion %c.%c.%c\n"
                           "\tTM32U585AI-SensorTile.box-Pro (%c) board"
                           "\n",
                           ATRBLE1_PACKAGENAME,
...
static void ExtConfigSetNameCommandCallback(uint8_t *NewName)
{
  ...
  if (FinishGood == FINISHA)
  {
    UpdateCurrFlashBankFwIdBoardName(ATRBLE1A_BLUEST_SDK_FW_ID, NewName);
  }
  else
  {
    UpdateCurrFlashBankFwIdBoardName(ATRBLE1B_BLUEST_SDK_FW_ID, NewName);
  }
