...
#define ATRBLE1_END_ADDR_COMPACT_DATA   (STSMART_NFC_MAX_SIZE - 4)
...
/* Max PayLoad Size == NFC TAG Size - 4 for Terminator TLV (1 byte for TLV)
                                    ...
                                    - ATRBLE1_UID_EXTRA_LENGHT for SmarTag UID
* And because we write 8 bytes for each sample... it must be a multiply of 8!
*/
#define ATRBLE1_MAX_PAYLOAD_SIZE (((STSMART_NFC_MAX_SIZE-4-ST25DV_CC_SIZE-4-6-SMART_TYPE_STRING_LENGTH-ATRBLE1_UID_EXTRA_LENGHT)>>3)<<3)
...
void NfcType5_NDEFInitHeader(void)
{
  ...
  {
    ATRBLE1_PRINTF("SmarTagUID= %lx%lx\r\n", pUid.MsbUid, pUid.LsbUid);
  }
  ...
  */
  ...
  {
    uint8_t NDEFHeader[20 + ATRBLE1_UID_EXTRA_LENGHT];
    ...
    /* NDEF HEADER */
    ATRBLE1_PRINTF("Creating NDEF External record Type for saving log data\r\n");
    ATRBLE1_PRINTF("\tST NFC Protocol Ver %d Rev %d\r\n", ATRBLE1_RECORD_VERSION, ATRBLE1_RECORD_REVISION);
    ...
    NDEFHeader[1 ] = SMART_TYPE_STRING_LENGTH; /* Type length */
    NDEFHeader[2 ] = 0x00; /* Playload Length MSB*/
    NDEFHeader[3 ] = 0x00;
    NDEFHeader[4 ] = 0x00;
    NDEFHeader[5 ] = 0xFF - SMART_TYPE_STRING_LENGTH - 6 - ATRBLE1_UID_EXTRA_LENGHT; /* Playload Length LSB*/
    ...
    if (BSP_NFCTAG_WriteData(BSP_NFCTAG_INSTANCE, NDEFHeader, ST25DV_CC_SIZE /* CC */ + 4 /* TTLV */,
                             20 + ATRBLE1_UID_EXTRA_LENGHT) != NFCTAG_OK)
    {
      Error_Handler(ATRBLE1_ERROR_NFC_WRITING, __FILE__, __LINE__);
    }
  }
...
void NfcType5_NDEFUpdateHeaderAddTermTLV(uint32_t BytesAdded)
{
  ...
  if (NDEFPayLoadLength <= ATRBLE1_MAX_PAYLOAD_SIZE)
  {
    uint8_t NDEFHeaderSection[2];
    uint32_t SizeForTLV;

    SizeForTLV = NDEFPayLoadLength + SMART_TYPE_STRING_LENGTH + 6 + ATRBLE1_UID_EXTRA_LENGHT /* For NDEF header */;
    ...
    if (NDEFPayLoadLength > 0xFF)
    {
      ...
      NDEFHeaderSection[1] = (NDEFPayLoadLength) & 0xFF;
      {
        ...
        {
          OnlyOneTime = 0;
          ATRBLE1_PRINTF("NDEFPayLoadLength>0xFF\r\n");
        }
      }
      /* Update NDEF header */
      ...
      {
        uint8_t CloseNDEF = NFCT5_TERMINATOR_TLV;
        if (BSP_NFCTAG_WriteData(BSP_NFCTAG_INSTANCE, &CloseNDEF, ST25DV_CC_SIZE /* CC */ + 4 /* TLV */ + SizeForTLV,
                                 1) != NFCTAG_OK)
        {
          Error_Handler(ATRBLE1_ERROR_NFC_WRITING, __FILE__, __LINE__);
        }
...
void NfcType5_SetInitialNDEFPayLoadLengthValue(uint32_t Address)
{
  ...
  NDEFPayLoadLength = Address - 4 - ATRBLE1_START_ADDR_OFFSET;
  ...
  MaxSamplesNumber = (ATRBLE1_END_ADDR_COMPACT_DATA - BeginAddrCompactData) >> 3;

  ATRBLE1_PRINTF("\r\nNfcType5_SetInitialNDEFPayLoadLengthValue:\r\n");
  ATRBLE1_PRINTF("\tNDEFPayLoadLength=%d\r\n", NDEFPayLoadLength);
  ATRBLE1_PRINTF("\tBeginAddrCompactData=%d\r\n", BeginAddrCompactData);
  ATRBLE1_PRINTF("\tEndAddrCompactData=%d\r\n", ATRBLE1_END_ADDR_COMPACT_DATA);
  ATRBLE1_PRINTF("\tMaxSamplesNumber=%d\r\n", MaxSamplesNumber);
}
...
void NfcType5_UpdateSampleCounter(SNFC_LogDefinition_t *LogDef, uint32_t BytesAdded)
{
  ...
  else
  {
    ...
    {
      OnlyOneTime = 0;
      ATRBLE1_PRINTF("----->Stop Updating Sample Counter\r\n");
    }
  }

  if (LogDef->LastSamplePointer >= ATRBLE1_END_ADDR_COMPACT_DATA)
  {
...
void NfcType5_ComputeNDEFPayLoadSize(uint32_t LastSamplePointer, uint32_t SampleCounter)
{
  ...
  {
    NDEFPayLoadLength = ATRBLE1_MAX_PAYLOAD_SIZE;
  }
  else
  {
    NDEFPayLoadLength = (LastSamplePointer - ATRBLE1_START_ADDR_OFFSET);
  }
}
